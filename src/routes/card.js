...
router.post('/storecode/productocarrito',(req,res)=>{
    const {idProducto,idUsuario,cantidadProducto}= req.body;
    const query= `CALL psInsProductoCarritov2(${idProducto},${idUsuario},${cantidadProducto})`;
...
router.get('/storecode/productsInCard/:id',(req,res)=>{
    const {id} = req.params;
    const query= `CALL psMosProdInCard(${id})`;
